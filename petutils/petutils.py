...
def collect_anat_and_pet(
    ...
    if type(bids_data) is BIDSLayout:
        ...
        isinstance(bids_data, (pathlib.PosixPath, pathlib.WindowsPath))
        and bids_data.exists()
    ):
        bids_data = BIDSLayout(bids_data)
    else:
        raise TypeError(
            f"{bids_data} must be a BIDSLayout or valid Path object, given type: {type(bids_data)}."
        )
    ...
    for subject in subjects:
        ...
        anat_files = [
            a.path
            for a in bids_data.get(
                suffix=suffixes, subject=subject, extension=["nii", "nii.gz"]
            )
        ]
        ...
        for entry in pet_files:
            if type(entry) is BIDSImageFile:
                try:
                    ...
                    mapped_pet_to_anat[subject][entry.path] = get_close_matches(
                        entry.path, anat_files, n=1
                    )[0]
                except IndexError:
...
def check_nifti_json_frame_consistency(
    bids_data: Union[pathlib.Path, BIDSLayout], subjects: list = []
):
    """
    ...
    for subject in subjects:
        ...
        pet_files = bids_data.get(
            subject=subject, suffix="pet", extension=["nii", "nii.gz"]
        )
        for entry in pet_files:
            if type(entry) is BIDSImageFile:
                ...
                frame_timings = {
                    "FrameTimesStart": len(entry.entities["FrameTimesStart"]),
                    "FrameDuration": len(entry.entities["FrameDuration"]),
                }
                if frame_timings["FrameTimesStart"] != frame_timings["FrameDuration"]:
                    error_string.append(
                        f"Number of entries for FrameTimesStart -> {frame_timings['FrameTimesStart']} and FrameDuration -> {frame_timings['FrameDuration']} do not match in {entry_json}"
                    )
                    inconsistent_files[subject]["files"][entry.path] = entry_json
                if frame_timings["FrameTimesStart"] != nii_frames:
                    error_string.append(
                        f"Number frames in {entry.path} header -> {nii_frames} does not match the number of frames in FrameTimesStart -> {frame_timings['FrameTimesStart']} at {entry_json}"
                    )
                    inconsistent_files[subject]["files"][entry.path] = entry_json
                if frame_timings["FrameDuration"] != nii_frames:
                    error_string.append(
                        f"Number frames in {entry.path} header -> {nii_frames} does not match the number of frames in FrameDuration -> {frame_timings['FrameDuration']} at {entry_json}"
                    )
                    inconsistent_files[subject]["files"][entry.path] = entry_json
